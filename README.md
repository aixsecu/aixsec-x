...
**Kiểm tra trước khi chạy agent** (bắt được 3 lỗi: chưa bind 0.0.0.0 / firewall /
model chưa pull):
...
[i] Model trên server (2): qwen2.5:7b, llama3.2:3b
[✓] WEBX_MODEL='qwen2.5:7b' CÓ trên server — sẵn sàng dùng.
```

Lỗi `KHÔNG kết nối được Ollama` → máy chủ chưa set `OLLAMA_HOST=0.0.0.0` (Cách B)
hoặc firewall chưa mở `11434/tcp`. Lỗi `WEBX_MODEL ... KHÔNG có trên server` →
chạy `ollama pull qwen2.5:7b` trên MÁY CHỦ.

### Dependencies + system tools (chạy trên Kali, áp dụng cho cả 4 cách)
...
```
[*] CHƯA khai báo target web (WEBX_TARGETS).
    Nhập target được ủy quyền ... để TRỐNG ... nếu phiên này CHỈ phân tích source code:
aixsec-target> https://abc.vn
[*] CHƯA khai báo thư mục source (WEBX_SRC_DIRS).
    Nhập thư mục code được phép quét SAST ... để TRỐNG ... nếu không dùng sast_scan:
aixsec-src> /var/www/html
```

3 kịch bản đều OK: **chỉ web** (src để trống), **chỉ SAST** (target để trống), **cả 2**.
...
# [*] Recon xong → agent đã có probe + headers
# aixsec-x> "Phân tích và tìm lỗ hổng"
# → agent: http_probe → detect_cms → waf_detect → nuclei (severity high) ...
# → approval prompt: "[APPROVAL] 'nuclei_scan' mức rủi ro [active] — cho phép? [y/N] y"
