...
`engine=mysql|mssql` (mssql **v1.4.5** = error-based oracle trước:
`' AND CONVERT(int,(SELECT ...))-- -` leaks the value inside the 500 conversion
error — 0-delay; fallback WAITFOR DELAY). POST search forms: pass
`method:'post', param:'keyword', data:'keyword=tin tuc'` (mode=form).
