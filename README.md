...
| wapiti_scan | active | noisy | **v1.5.0:** full-site scanner (wapiti 3.2.10) — ALL 29 attack modules passed via `-m` (default wapiti runs only 9); bounded: scope `url\|page\|folder\|subdomain\|domain\|punk`, `depth` 1–10, `max_scan_time`/`max_attack_time`, `tasks` 1–8, `timeout`; `-f json` report parsed (severity+category sorted, wstg, `curl_command`, wapiti probe-marker `%C2%BF%27%22%28` stripped, CRLF-safe body extraction); `exploit=true` (default) → sqlmap-FIRST: `sqlmap_runner` on ≤3 SQLi findings after wapiti CONFIRMS (technique E/T, dbms from `DBMS:` info); per-category guidance map — CSP/headers/cookie-flag items are report CATEGORIES, not modules |
